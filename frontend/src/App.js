...
function App() {
  ...
  const updateItem = async (e) => {
    e.preventDefault();
    try {
      await api.put(`/items/${editItem.id}`, { name: editItem.name, description: editItem.description });
            ...
            {editItem.id === item.id ? (
              <form onSubmit={updateItem} className="flex flex-col space-y-2">
                ...
                <div>
                  ...
                  <button onClick={() => setEditItem({ id: null, name: '', description: '' })} className="bg-gray-500 text-white p-1 rounded">Cancel</button>
                </div>
              </form>
            ) : (
              <>
                ...
                <div className="mt-2">
                  <button onClick={() => setEditItem({ id: item.id, name: item.name, description: item.description })} className="bg-yellow-500 text-white p-1 rounded mr-2">Edit</button>
